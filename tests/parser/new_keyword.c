...
START_TEST(parse_valid_new_keyword)
  ...
  ck_assert_ptr_ne(caller.data, NULL);
  ck_assert_wstr_eq(caller.data, L"String");
  ...
  ck_assert(caller.tokenType == TS_CALLER);
  ck_assert_ptr_ne(caller.data, NULL);
  ck_assert_wstr_eq(caller.data, L"Function");
  ck_assert_uint_eq(caller.childrenSize, 0);
  ck_assert_ptr_eq(caller.children, NULL);
  ...
  ck_assert(caller.tokenType == TS_CALLER);
  ck_assert_ptr_ne(caller.data, NULL);
  ck_assert_wstr_eq(caller.data, L"Array");
  ...
  ck_assert_wstr_eq(arg.data, L"1");
  arg = caller.children[1];
  ck_assert(arg.tokenType == TS_ARGUMENT);
  ck_assert_ptr_ne(arg.data, NULL);
  ck_assert_wstr_eq(arg.data, L"2");
  arg = caller.children[2];
  ck_assert(arg.tokenType == TS_ARGUMENT);
  ck_assert_ptr_ne(arg.data, NULL);
  ck_assert_wstr_eq(arg.data, L"3");
